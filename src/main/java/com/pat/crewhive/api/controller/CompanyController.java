...
public class CompanyController implements CompanyControllerInterface {
    ...
    public ResponseEntity<List<UserIdAndUsernameDTO>> getCompanyUsers(@AuthenticationPrincipal CustomUserDetails cud,
                                                                   @PathVariable @NotNull Long companyId) {

        Long managerId = cud.getUserId();

        List<UserIdAndUsernameDTO> users = companyService.getAllUsersInCompany(managerId, companyId);
